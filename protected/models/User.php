...
class User extends CActiveRecord
{
	...
	public function checkUnique($attribute,$params) {
	    ...
	    if ( $count > 0 )
		$this->addError($attribute,"Nazwa użytkownika jest zajęta!"); 
	    else
		return true;
	...
	function CheckStrength($attribute,$params) 
	{
		...
		if ($strength<$params['score']) 
		        $this->addError($attribute,"Hasło jest słabe! Proszę użyć znaków specjalnych!. Twoja siła hasła: ".$strength." z minimum ".$params['score']); 
		else
		    return true;
	}
	/// do obczajenia
