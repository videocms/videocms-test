...
class CmsvideoUsers extends CActiveRecord
{
 ...
 return array(
  ...
  array('email','email'),
  array('user_login, user_pass, email', 'required'),
  array('role, status, pagination', 'numerical', 'integerOnly'=>true),
  array('user_login, firstname, lastname', 'length', 'max'=>128),
  array('user_pass, email', 'length', 'max'=>128),
  array('last_login_time, create_date', 'safe'),
  // The following rule is used by search().
  // Please remove those attributes that should not be searched.
  array('user_id, user_login, user_pass, email, pagination, createtime, last_login_time, role, status', 'safe', 'on'=>'search'),
  );
 }
 ...
 if ($this->isNewRecord) {
     $this->user_pass = md5($this->passwordSave); 
